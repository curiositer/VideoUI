...
- **多画面轮播**：多个摄像头按 `cameraRotateInterval` 秒（默认 30）循环切换；间隔为 0 或只有一个摄像头时固定播放。轮播 tick 触发时若连接切换进行中（`switchLocked`）则跳过本轮、索引不推进，避免打断故障切换链
- **主备故障切换**：每个摄像头可配多个备用流（`backups` 列表）。当前画面断流（帧数看门狗约 20 秒检测 / WebRTC 连接失败 / WHEP 握手失败 / 本地视频加载失败 / 连接就绪超时——备用流与首次直接连接均 8 秒无画面）时按链切换：`主画面 → 备1 → 备2 → … → 下一个摄像头的主画面`；只有一个摄像头且无备用时原地重建（自愈）。就绪超时且 standby 无画面内容时保持当前画面不渐变，避免切换黑屏
